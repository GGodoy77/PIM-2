...
class TeamAcreate(CTkFrame):
    def __init__(self, master):
        ...
        CTkLabel(form, text='Nome do Time', font=CTkFont(size=16)).pack(pady=(30, 1))
        self.entry_usuário=CTkEntry(form, placeholder_text="Digite o nome do seu time", fg_color='white',text_color='black',width=250)
        self.entry_usuário.pack(pady=10)
    ...
    def senha_valida(self, senha):
        self.letras=re.findall(r'[A-za-z]', senha)
        self.tem_maiuscula=re.search(r'[A-Z]', senha)
    ...
    def cadastrar(self):
        ...
        if not self.email or not self.confirmemail or not self.senha or not self.confirmsenha or not self.nome:
            ...
            messagebox.showerror("Erro", "A senha deve conter pelo menos 4 letras, uma letra maiúscula e um caractere especial.\nObs:Os caracteres especiais não contarão como letras!")
